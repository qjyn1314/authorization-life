    ...
    public void logout(HttpServletRequest request, HttpServletResponse response, Authentication authentication) {
        ...
        try {
            ...
            if (Objects.nonNull(userDetail)) {
                ...
                String cacheUserToken = Formatter.of(SecurityConstant.USER_DETAIL).add("token", userToken).format();
                redisHelper.delete(cacheUserToken);
                redisHelper.delete(Formatter.of(SecurityConstant.TOKEN_STORE).add("userId", userDetail.getUserId().toString()).format());
            }
            ...
            if (StrUtil.isBlank(token)) {
                ...
                out.write(JSONUtil.toJsonStr(new Result<>(Result.ERROR, "未找到token，请确认已登录。", null)));
                out.flush();
                out.close();
            }
            token = token.split(" ")[1];
            ...
            PrintWriter out = response.getWriter();
            out.write(JSONUtil.toJsonStr(new Result<>(Result.SUCCESS, Result.SUCCESS_DESC, null)));
            out.flush();
            out.close();
        } catch (IOException e) {
