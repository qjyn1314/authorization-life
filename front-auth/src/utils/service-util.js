    ...
    if (error.response) {
        ...
        switch (status) {
            case 400:
                ...
                prompt.error(error.response.data.message || '参数校验失败')
            ...
            case 401:
                ...
                if (error.response.data.message === 'Token不存在或已过期') {
                    router.replace('/')
                    prompt.error('账号已过期,请重新登录')
                    return Promise.reject({error: 'Unauthorized', message: '账号已过期,请重新登录'});
                } else {
                    prompt.error(error.response.data.message || '账号已过期,请重新登录')
