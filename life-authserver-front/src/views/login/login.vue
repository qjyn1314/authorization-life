...
export default {
  ...
  methods: {
    ...
    getRedirectUri () {
      console.log("已登录成功...")
      console.log("已登录成功..."+`${LOGINTOKEN.loginRedirectUrl}/${AUTH_SERVER}/oauth2/authorize?client_id=${this.ruleForm.client_id}&response_type=${LOGINTOKEN.response_type}&redirect_uri=${this.redirect_uri ? encodeURIComponent(this.redirect_uri) : encodeURIComponent(LOGINTOKEN.redirect_uri)}`)
