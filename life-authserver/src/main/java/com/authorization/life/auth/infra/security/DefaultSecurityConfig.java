...
public class DefaultSecurityConfig {
  ...
  @Autowired private RegisteredClientService registeredClientService;
  ...
  public SecurityFilterChain defaultSecurityFilterChain(
    ...
    http.logout(
        logout ->
            logout
                ...
                .addLogoutHandler(
                    new SsoLogoutHandle(
                        authorizationService, redisUtil, ssoSecurityProperties, jwtService))
                // 在此处可以删除相应的cookie
    ...
    // 其目的主要是为了每次验证解析jwtToken为当前登录用户并设置到当前sesurity会话中
  ...
  /**
   * 配置身份验证提供程序, 不同的验证请求策略判断, 通过 AbstractUserDetailsAuthenticationProvider#supports 方法确定,
   * 此处可以定义多个身份验证实现器
   */
  @Bean
  public AuthenticationManager authenticationManager(
      HttpSecurity http, AuthenticationProvider usernamePasswordProvider) {
