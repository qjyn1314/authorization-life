...
public class SsoLogoutHandle implements LogoutHandler {
  ...
  public SsoLogoutHandle(
      OAuth2AuthorizationService oAuth2AuthorizationService,
      RedisUtil redisUtil,
      SsoSecurityProperties ssoSecurityProperties,
  ...
  public void logout(
    ...
    if (StrUtil.isBlank(authorization) || StrUtil.isBlank(interiorJwt)) {
      try {
        ...
        out.write(
            JSON.toJSONString(
                Result.failCode(Result.ERROR, SecurityErrorEnums.LOGIN_IS_INVALID.formatMsg())));
        out.flush();
        out.close();
      } catch (Exception e) {
    ...
    // 此处重新解析是由于 SsoLogoutHandle 在  JwtAuthenticationFilter 之前执行,此时并没有将当前登录用户设置到
    // SecurityContextHolder 中.
    UserDetail userDetail =
        BeanUtil.toBean(jwtService.getClaimsFromJwtToken(interiorJwt), UserDetail.class);
    ...
    if (StrUtil.isNotBlank(userDetail.getAccessTokenId())) {
      OAuth2Authorization auth2Authorization =
          oAuth2AuthorizationService.findById(userDetail.getAccessTokenId());
      ...
      OAuth2Authorization auth2Authorization =
          oAuth2AuthorizationService.findByToken(
              StrUtil.removePrefixIgnoreCase(authorization, SsoSecurityProperties.ACCESS_TOKEN_TYPE)
                  .trim(),
    ...
    try {
      PrintWriter out = response.getWriter();
      out.write(JSON.toJSONString(Result.ok()));
      out.flush();
      out.close();
    } catch (IOException e) {
