...
public class SsoSuccessHandler implements AuthenticationSuccessHandler {
  ...
  public void onAuthenticationSuccess(
      ...
      throws IOException {
    log.info("密码验证登录成功的对象信息是-{}", JSON.toJSONString(authentication));
  ...
  private void authorizationCodeUrl(HttpServletRequest request, Authentication authentication) {
    ...
    CaptchaWebAuthenticationDetails details =
        (CaptchaWebAuthenticationDetails) authentication.getDetails();
    ...
    String redirectUrl =
        genAuthorizationCodeUrl(
            hostOrigin,
            oauthClientVO.getClientId(),
            oauthClientVO.getScopes(),
            state,
            oauthClientVO.getRedirectUri());
   ...
   *
   * @param hostOrigin 授权服务器域名前缀,例如: <a
   ...
   * @return String
   */
  String genAuthorizationCodeUrl(
      String hostOrigin, String clientId, String scope, String state, String redirectUri) {
    return hostOrigin
