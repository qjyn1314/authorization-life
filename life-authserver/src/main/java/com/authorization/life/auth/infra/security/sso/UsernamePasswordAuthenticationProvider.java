...
public class UsernamePasswordAuthenticationProvider
  ...
  protected void additionalAuthenticationChecks(
    ...
    if (passwordEncoder.matches(presentedPassword, userDetails.getPassword())) {
      // 清除密码错误次数累计
      String cacheKey =
          StringUtil.of(PASSWORD_ERROR_COUNT).add("username", userDetails.getUsername()).format();
      ...
      int passwordErrorCount =
          Optional.ofNullable(redisUtil.get(cacheKey)).map(Integer::parseInt).orElse(0);
    ...
    if (authenticationDetails instanceof CaptchaWebAuthenticationDetails clientIdDetails) {
      ...
      Assert.notBlank(
          clientIdDetails.getClientId(), () -> new OAuth2AuthenticationException("客户端ID不能为空."));
      ...
      // 此处为Oauth2.0的授权码模式  https://datatracker.ietf.org/doc/html/rfc6749#section-4.1
      registeredClientService.checkClient(
          registeredClient, AuthorizationGrantType.AUTHORIZATION_CODE, null, null);
    }
  }
}
