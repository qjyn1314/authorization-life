...
public class JwtAuthenticationFilter extends OncePerRequestFilter implements InitializingBean {
  ...
  public JwtAuthenticationFilter(
      SsoSecurityProperties ssoSecurityProperties, JwtService jwtService) {
  ...
  protected void doFilterInternal(
    ...
    log.info("请求路径是-{}", JSON.toJSONString(request.getRequestURI()));
    ...
    if (!ssoSecurityProperties.getEnable()) {
      ...
      chain.doFilter(request, response);
      log.info(
          "将退出-JwtAuthenticationFilter-过滤器-jwtToken->>>{}",
          JSON.toJSONString(UserDetailUtil.getUserContext()));
    }
    // 将在此次请求结束后清空此缓存的用户信息
  ...
  public UserDetail getUserDetailByInteriorJwt(String interiorJwt) {
    ...
    return CollUtil.isNotEmpty(fromJwtToken)
        ? BeanUtil.toBean(fromJwtToken, UserDetail.class)
        : null;
  }
}
